...
│   ├── skill-author/ · harness-factory/ · asset-factory/
│   ├── grill-me/
...
5. **자가검증 (slot ≠ purpose)** — grill-me·meta-skill axes·soft-routing 모두 slot 채우기 금지, 사용자 plan에 실제 영향 있을 때만 출력.
6. **회귀 0** — 모든 release는 118+ unit tests + contract tests 통과 필수. baseline snapshot은 외부 사용자가 실측.

### 11.3 6 아키텍처 패턴 (harness-factory)

`/cfh-team` + `harness-factory`가 생성하는 multi-agent 팀의 6 패턴 (자세한 결정 트리는 `skills/harness-factory/SKILL.md`):
