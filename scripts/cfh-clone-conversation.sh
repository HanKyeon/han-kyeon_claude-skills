#!/usr/bin/env bash
#
# cfh-clone-conversation.sh - Clone the tail of a Claude Code conversation from a CFH-aware cut point
# cfh-clone-conversation.sh - Clone the tail of a Claude Code conversation from a CFH-aware cut point
# Pure bash implementation - no Python/Node dependencies.
# Works on macOS (bash 3.2+) and Linux.
#
# Usage:
#   cfh-clone-conversation.sh <session-id> [project-path]
#
# Arguments:
...
#
# Example:
#   cfh-clone-conversation.sh d96c899d-7501-4e81-a31b-e0095bb3b501
#   cfh-clone-conversation.sh d96c899d-7501-4e81-a31b-e0095bb3b501 /home/user/myproject
#
...
convert_path_to_dirname() {
    local path="$1"
    # On MINGW/Windows, convert /c/... to C:/... to match Claude Code's internal format
    if [[ "$path" =~ ^/([a-zA-Z])/ ]]; then
        local drive="${BASH_REMATCH[1]}"
        drive=$(echo "$drive" | tr '[:lower:]' '[:upper:]')
        path="${drive}:${path:2}"
    fi
    # Replace :, /, \ with - (matches Claude Code's dirname conversion)
    echo "$path" | sed 's|[:/\\]|-|g'
}

# Convert a MINGW/Git-Bash path (/c/labs/mes) to a native Windows path (C:\labs\mes)
# so history.jsonl's "project" field matches Claude Code's native project key.
# /resume filters resumable sessions by that key; without this, Windows clones
# are filed under /c/... and never appear. No-op on macOS/Linux (no drive prefix).
to_native_project_path() {
    local path="$1"
    if [[ "$path" =~ ^/([a-zA-Z])/(.*)$ ]]; then
        local drive="${BASH_REMATCH[1]}"
        drive=$(echo "$drive" | tr '[:lower:]' '[:upper:]')
        path="${drive}:/${BASH_REMATCH[2]}"
        path="${path//\//\\}"   # forward slashes -> backslashes
    fi
    echo "$path"
}
...
pre_generate_uuids() {
    ...
    if command -v python &> /dev/null && python --version &> /dev/null; then
        # Python (Windows/MINGW 최우선, 크로스 플랫폼)
        # Windows Store alias 문제로 python3보다 python을 우선 시도
        local py_cmd
        py_cmd=$(command -v python 2>/dev/null)
        "$py_cmd" -c "
import uuid
...
" > "$output_file"
        ...
        local py_cmd
        py_cmd=$(command -v python3 2>/dev/null)
        "$py_cmd" -c "
import uuid
for _ in range($count):
    print(uuid.uuid4())
" > "$output_file"
        ...
        log_error "No UUID generation method available (need python, hexdump, /proc/sys/kernel/random/uuid, or uuidgen)"
        return 1
    fi
}
...
cfh_clone_conversation() {
    ...
    local clone_tag="[CFH-CLONE ${clone_timestamp}]"
    ...
    if [ "$total_clean_user_messages" -lt 2 ]; then
        log_error "Conversation has fewer than 2 clean user messages, nothing to clone"
        exit 1
    fi
    ...
    if [ -z "$skip_clean_count" ]; then
        log_warning "No CFH cut anchor (sentinel/cfh command) found or cfh CLI unavailable — falling back to halfway cut"
        skip_clean_count=$((total_clean_user_messages / 2))
    else
        log_info "CFH cut anchor: ${cut_reason}${cut_trim_pct:+ (trims ~${cut_trim_pct}% of lines)}"
    fi
    ...
    mkdir -p "$project_dir"
    log_info "Cloning conversation tail to: $target_file"

    # OPTIMIZED First pass: find if last clean user message is a clone command
    ...
    if [ -n "$clean_user_lines" ]; then
        ...
        if echo "$last_line_info" | grep -qE '<command-message>(dx:)?clone</command-message>|<command-message>(dx:)?cfh-clone</command-message>' 2>/dev/null; then
...

    {
        ...
        # Prepend clone_tag to customTitle (shown in /resume list)
        # Only prepend if not already prefixed to avoid duplicate tags
        if (match(line, /"customTitle":"[^"]*"/)) {
            title_match = substr(line, RSTART, RLENGTH)
            if (index(title_match, "[CFH-CLONE") == 0) {
                gsub("\"customTitle\":\"", "\"customTitle\":\"" clone_tag " ", line)
            }
        }
    ...
    local ref_text="The cfh-clone is complete. This conversation preserves the tail from the CFH-aware cut point (anchor: ${cut_reason}) - earlier context was removed to free up space. The handover card and the latest confirmed intent are included above. Just continue working from where you left off. Original session: \`${source_session}\` at: ${source_file}"
    ...
    display_text=$(tail -n +"$((skip_count + 1))" "$source_file" | filter_clean_user_msgs | head -1 | \
        ...
        head -c 200 || echo "[CFH-cloned conversation]")
    ...
    # Add history entry — project must use Claude Code's native key (Windows: C:\..).
    local native_project native_project_json
    native_project=$(to_native_project_path "$project_path")
    native_project_json=$(printf '%s' "$native_project" | sed 's/\\/\\\\/g')   # \ -> \\ for JSON
    echo "{\"display\":\"${display_text}\",\"pastedContents\":{},\"timestamp\":${timestamp},\"project\":\"${native_project_json}\",\"sessionId\":\"${new_session}\"}" >> "$HISTORY_FILE"
    log_success "History entry added"

    # Note: We don't copy todos for cfh-clone since the context is truncated

    log_success "Conversation tail cloned successfully!"
    echo ""
...
if [ "$PREVIEW_MODE" = true ]; then
    ...
    cfh_clone_conversation "$SESSION_ID" "$PROJECT_PATH"
fi
