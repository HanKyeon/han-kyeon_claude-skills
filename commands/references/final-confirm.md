...
```
📋 최종 의도 해석 (Final Intent Confirm) <<cfh:final-confirm>>
...
## 절단점 sentinel — `<<cfh:final-confirm>>`

카드 머리의 고정 마커. **세션 클론 도구(`cfh clone-cut` 등)가 "확정된 의도"의 위치를 grep 한 번으로 찾는 앵커**다 — 클론 시 이 카드 이후가 보존된다.

- **변형 금지** — 철자·괄호 그대로. 카드 첫 줄에만.
- **카드 외 prose에 이 마커를 출력하지 말 것** — assistant 메시지의 마커는 전부 "확정 카드"로 간주되므로, 설명·예시에서 언급해야 하면 마커를 쪼개거나 우회 표기.
- 탐지는 assistant 메시지에서만 인정 — 본 문서를 Read한 tool_result에 마커가 보여도 오탐하지 않음.
