...
### Telemetry: disabled — 인라인 y/n 프롬프트

```
⚠️ 텔레메트리가 비활성화 상태입니다. 피드백 기록을 위해 활성화가 필요합니다.

활성화하면 `~/.claude/.cfh-logs/`에만 로컬 저장됩니다 — 외부 전송 없음.

지금 활성화하시겠습니까? [y/n]
```

**(y) 선택 시**:
1. 사용자 동의 기록 → `cfh log --enable` 실행
2. "✅ 텔레메트리 활성화됨" 알림
3. Step 2로 진행

**(n) 선택 시**:
1. "이번 피드백은 영구 기록되지 않지만 이 세션 내에서는 참고용으로 남기겠습니다" 안내
2. comment를 대화 컨텍스트(메모리)에만 유지
3. Step 2는 skip. 종료 보고에 "세션 내 참고" 상태임을 명시

**무응답·취소**: "피드백 수집을 다음 기회로 미루겠습니다" + 종료. 기록 없음.

### 출력 예

```
✅ 텔레메트리 활성화됨. 피드백 기록을 계속합니다.
```
또는
```
ℹ 세션 내 참고로만 남김: "<comment>" (영구 기록 안 됨)
```
