...
**동작**: `asset-factory` 메타-스킬 활성화 → Phase 0 기존 자산 스캔 → Phase 1 3 분류 질문(반복성/협업/트리거 방식) → Phase 2 적절한 메타-스킬(`skill-author`/`harness-factory`/인라인)로 위임.
...
**동작**: 4 Phase
- **Phase 0 Pre-scan** — CLAUDE.md·git log·package.json·대상 파일 자동 수집 (읽기 전용)
...
**동작**: `harness-factory` 메타-스킬 활성화 → Phase 0 Pre-scan → Phase 1 5 질문 + Deep-dive 옵트인(7 축 중 조건부) → Phase 2 6 패턴 중 1개 추천 → Phase 3~6 에이전트·스킬·시운전.
...
**관련**: `cfh generate <preset>` (3종 프리셋에 맞을 때 더 빠름), `/cfh-make` (팀 확정 안 됐을 때).
...
**관련**: `/cfh-tc` (기존 파일 테스트 보강), `cfh generate producer-reviewer` (오버핏 방지 팀 구성).
...
**관련**: `/cfh-tdd` (명시적 TDD만), 시나리오 가이드 3번(보강) · 4번(TDD).
...
- `~/.claude/skills/skill-author/SKILL.md` — 스킬 작성 메타-스킬의 6 Phase 상세 (Pre-scan + 조건부 follow-up + Sanity check)
- `~/.claude/skills/harness-factory/SKILL.md` — 팀 생성 메타-스킬 + 6 패턴 문서 (Pre-scan + Deep-dive + Sanity check)
- `~/.claude/skills/asset-factory/SKILL.md` — Dispatcher 3 Phase + 분류 트리
