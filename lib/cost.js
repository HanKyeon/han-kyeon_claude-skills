...
// Git commit hash is hex 4~40 chars. Reject anything else to prevent shell injection
// via `cfh stats cost --since-commit "abc; rm -rf ~"` (CWE-78).
...
function getCommitTimestamp(commitHash, cwd = process.cwd()) {
  if (!commitHash || typeof commitHash !== 'string' || !COMMIT_HASH_RE.test(commitHash)) {
    return null;
  }
  try {
    const res = spawnSync('git', ['-C', cwd, 'show', '-s', '--format=%cI', commitHash], {
      encoding: 'utf8',
