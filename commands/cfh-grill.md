...
- **자동 컨텍스트 흡수 안 함** — 직전 turn에 무엇이 있든 사용자가 명시 인자/답변을 줘야 진행 (ambiguous 응답 원칙과 일관)
...
**영향·의존성 가지에서 정형 데이터 크로스체크** (→ `commands/references/structured-crosscheck.md`): grill이 *"이 변경 영향이 X뿐?"* 같은 영향·의존성 결정 가지를 팔 때, 추론에 기대지 말고 **Grep으로 실제 참조를 대조해 반증**한다 — `"네 주장과 달리 grep엔 Y·Z도 참조 [verified]"`. 이는 grill의 *반증(adversarial)* 정신과 정확히 일치. 누락은 severity 분기(직접 참조=high), 한계(grep 정적·동적 못 잡음)는 명시. 영향과 무관한 결정 가지(예: 네이밍·UX 취향)에는 *적용 안 함* (slot ≠ purpose).
...
💡 Team 활용 가치 (grill 결과 기반 — 신호 medium+ 시만 출력)
   📊 신호 분석:
