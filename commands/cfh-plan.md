<s>
이 커맨드는 복잡하거나 모호한 작업을 시작할 때 **목표 캡처 → 접근법 상의 → 작업 분류 → 실행·위임**의 4 Phase를 수행합니다.
...
## Phase 0 — Pre-scan (자동 수집)

사용자에게 질문하기 전에 프로젝트 맥락을 훑습니다. 읽기 전용, 파일 생성·수정 금지.

**수집 대상**:
1. `CLAUDE.md` / `.cursorrules` / `CONTRIBUTING.md` — 프로젝트 규약
2. `package.json` — 스택·scripts (test/lint/build/typecheck 명령 확인)
3. 최근 git log (`git log --oneline -20`) — 진행 중 테마
4. 변경 중인 파일 (`git status --short`) — 현재 작업 트리
5. 대상 파일·디렉터리 존재 여부 (`$ARGUMENTS`에 경로가 있으면 Glob/Read)
6. (선택, `gh` CLI 있으면) 열린 PR·Issue 중 관련 있을 만한 것
...
- 최근 진행 중: <최근 5 커밋 주제 한 줄 요약>
- 작업 트리: <clean / N files modified>
- 대상 존재: <파일/디렉터리 경로, 상태>

위 정보 기반으로 Phase 1을 진행합니다. 잘못된 부분 있으면 알려 주세요.
```

Pre-scan 결과는 **이후 Phase에서 재질문하지 않도록** 저장. 예: 테스트 명령을 Phase 3 실행 계획에서 자동으로 사용.

## Phase 1 — Goal Capture (4 질문)

**기본 원칙**: 한 번에 한두 질문씩. 답변 불충분하면 옵션 제시로 재질문.
...
`$ARGUMENTS`에 목표가 있으면 "이대로 이해했는데 맞으십니까?"로 확인만.
...
Phase 0·1의 정보를 근거로 Claude가 **접근법 카드**를 제시합니다. 사용자 승인 전 실행 금지.
...
각 Phase 종료 시 사용자에게 짧게 보고:
- **Phase 0 완료**: Pre-scan 카드 (잘못된 부분 교정 기회)
- **Phase 1 완료**: 4개 답변 요약 + "Phase 2로 진행해도 될까요?"
- **Phase 2 완료**: 접근법 카드 (승인 대기)
- **Phase 3 완료**: 작업 완료 보고 (Q2 성공 기준 대비)
...
- Phase 1 답변이 모이기 전 코드 수정·파일 생성 금지.
