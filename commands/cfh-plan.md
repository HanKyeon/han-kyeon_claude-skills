...
- 값이 있으면 Phase 1 Q1의 초안으로 사용 후 Q2~Q5만 질문.
</invocation>
...
위 정보 기반으로 Q2~Q5를 진행합니다.
```

스캔 결과는 Phase 2·3에서 재스캔하지 않도록 저장.

### Step 1c — Q2~Q5 (스캔 결과 참고하며)
...
> *"이 작업을 시작하기 전에 끝나 있어야 할 것, 또는 지금 막고 있는 것이 있습니까?"*

**핵심: 빈 질문 금지 — agent가 먼저 파악·추천** (`commands/references/recommendation-pattern.md`). Step 1b Scoped scan 결과 + import·의존성 그래프를 근거로 *선행 후보를 먼저 제시*한 뒤 사용자 확인:

```
📌 선행 의존성 파악:
   - 🟢 [verified] <git status 미완료 / package.json 미설치 dep / 인용>
   - 🟡 [inferred] <import 그래프상 먼저 있어야 할 모듈·계약>
   - 🔴 [guessed] <약한 신호 — 사용자 확인 권장>

   추천: <발견된 선행 작업 N개 — 또는 "선행 의존성 없음, 바로 시작 가능">

다른 가능성 (사용자만 아는 것):
   - 다른 PR·API가 먼저 머지돼야 함
   - 환경변수·인프라 세팅 선행
   - 외부 팀 결정 대기 중

답변: 추천대로 / 추가 의존성 있음 <설명> / 없음 (바로 시작)
```

**답변 해석**:
- 선행 의존성 발견 → Phase 2 접근법에 *"먼저 X"* 선행 단계 추가, 또는 *"X 완료 후 재개"* 권장
- 차단 요소(blocker)면 → plan 범위를 *blocker 해소까지*로 한정 권장
- "없음" → 바로 진행

**Side-effect와의 대칭** (방향이 반대 — 중복 아님):
- **Q5 (inbound)**: *밖이* 이 작업에 거는 **선행조건** — 시작 전 확인
- **Phase 2.5 side-effect (outbound)**: 이 작업이 *밖에* 미치는 **영향** — 실행 직전 확인

**자가검증**: agent가 선행 의존성을 *발견 못 하고* 사용자도 "없음"이면 그대로 진행. "혹시 모르니" 가짜 의존성 만들지 말 것 (slot ≠ purpose).

## Phase 2 — Approach Proposal

Phase 1의 정보(목표 + scoped 스캔 결과 + Q2~Q5)를 근거로 Claude가 **접근법 카드**를 제시합니다. 사용자 승인 전 실행 금지.

### 태스크 분류

아래 카테고리 중 하나로 분류하고 근거를 공개:

| 분류 | 신호 | 권장 경로 (stack signal 포함) |
|---|---|---|
...
📌 추천: yes — Phase 3 실행
   이유:
     - 🟢 [verified] Q1~Q5 답변이 일관됨 (모순·공백 없음)
...
## Phase 2.5 — Final Intent Confirm (Phase 3 진입 직전)

Phase 2 Approach Card 승인 뒤, **Phase 3 Execution 시작 *직전*에 사용자 답변 모음을 합산 해석·모호 발화 검사·답변 충돌 자가검증** (→ `commands/references/final-confirm.md`).

**합산 대상**:
- Q1 목표 / Q2 성공 기준 / Q3 제약·out-of-scope / Q4 긴급도 / Q5 선행 의존성 — 각 답변 *짧게 인용*
- Phase 2 접근법 카드의 *위임 sub-command 결정* (예: `/cfh-tdd`·`/cfh-refactor` 등)
- *모호 발화* — 동음이의어·약어·도메인 가정 (예: "service" → Spring `@Service` bean으로 해석 등)
- *답변 간 충돌·gap* — 예: Q3 "API 변경 금지" vs Q2 "새 endpoint 추가" — 발견 시 명시

**검증 게이트** (자산별 핵심):
- 위임 sub-command 1개 명확
- Q3 out-of-scope 답 존재
- Q5 선행 의존성 *해소됨* 또는 *plan에 선행 단계 반영됨* (미해소 blocker면 그 사실 명시)
- Q1 목표에 *동사* + *명사 단위* 모두 있음 (모호하지 않음)
- **Side-effect 자가 확인** (큰 plan 시) — 아래 단락

### Side-Effect 자가 확인

큰 plan(다중 sub-command 위임·컨벤션 변경·인터페이스 변경 결정 포함)에서 *7 영역 카탈로그* 자가 확인 (→ `commands/references/progress-audit.md` § 6번째 축):

| 영역 | 검사 |
|---|---|
| 다른 결정 | 이 plan이 *이전 PROGRESS.md 결정*과 충돌? |
| 다른 자산 | 위임 sub-command가 *다른 skill·command*에 영향? |
| 인터페이스·계약 | API·SKILL.md description 변경 — *기존 사용자* 영향 명시? |
| 컨벤션·정책 | 새 정책이 *기존 코드*에 backwards-compat? |
| 의존성 | 추가·제거 dep이 *순환 의존*? |
| migration·schema | 변경이 *기존 데이터·환경* 영향? |
| 환경·설정 | settings·env 변경이 *다른 환경*(dev/prod·CI) 영향? |

**발견 처리**:
- *명시 안 한 영역*에 영향 가능성 발견 → Q3 out-of-scope에 *추가* 또는 plan 자체 정정
- 단순 plan(단일 sub-command·소규모 변경)이면 *생략* (자가검증 — slot ≠ purpose)
- `[verified]/[inferred]/[guessed]` 마커로 confidence 명시

**정형 데이터 크로스체크** (→ `commands/references/structured-crosscheck.md`):
변경 대상이 *다른 곳에서 참조되는 심볼·모듈·인터페이스*면, 추론한 영향 범위를 **Grep으로 실제 참조와 대조**해 누락을 잡는다:
- 추론 영향처 vs `grep -rn "<symbol>"` 실제 참조 → 누락 발견 시 severity 분기(직접 import=high→`[verified]`, 간접=medium, 동적/문자열=low)
- 정형 데이터(import graph·tsconfig refs 등)가 *있으면* 추가 활용
- **한계 명시 필수**: grep은 정적 — 동적 import·리플렉션 못 잡음. "참고용, 완결성 보장 아님"
- grep 대상 없는 격리 변경이면 *생략* (slot ≠ purpose)

**Adversary hint (조건부)**: side-effect 신호 강함(인터페이스 변경·다중 위임·컨벤션 변경 포함) 시 1줄 hint:
```
🔍 (옵션) side-effect를 깊이 검증하려면 — /cfh-grill 로 결정 트리·영향 가지를 파기
```

신호 약하면 *침묵* (자가검증).

답변: `yes` / `정정 <항목>` / `처음부터` / `pass` (보류) / `explain <항목>`. 짧은 동의(`OK`·`응`)는 ambiguous로 *대기*, 명시 `yes` 받기 전엔 Phase 3 진입 금지.
...
**직접 실행 시 자가 점검**:
- 모든 변경 전에 영향 범위 확인 — *추론만 X*, `commands/references/structured-crosscheck.md`로 Grep 대조해 누락 잡기
- 코드 품질: 리팩터성 변경은 `skills/refactoring-strategy` 방향 제안(대안·정량), 테스트 작성·수정은 `commands/references/test-quality.md`(리트머스·부작용) 적용
- Q3 out-of-scope 준수 여부 매 수정 후 체크
- 단계 완료마다 사용자에게 경계 보고

> 위임 경로(`/cfh-tdd`·`/cfh-refactor`·`/cfh-review`)면 해당 자산이 품질 기준을 *이미* 싣고 있어 전이 적용됨 — 직접 실행 경로에서만 위 2줄로 명시 연결.
...
- **Step 1c 완료**: Q2~Q5 답변 요약 + "Phase 2로 진행해도 될까요?"
- **Phase 2 완료**: 접근법 카드 (승인 대기)
- **Phase 2.5 완료**: Final Intent Confirm — 답변 합산·모호 발화·선행 의존성·side-effect 점검 후 **명시 yes 대기** (생략 금지)
...
- Phase 1 답변(목표·스캔·Q2~Q5)이 모이기 전 코드 수정·파일 생성 금지.
- Phase 2 승인 전 Phase 3 실행 금지.
- **Phase 2.5 Final Intent Confirm 생략 금지.** Phase 2 승인과 Phase 3 실행 *사이*에 반드시 발화 — 답변 합산·모호 발화·선행 의존성·side-effect 점검 후 명시 yes를 받기 전 Phase 3 진입 금지. (단순·소규모 작업도 *최소 1줄 합산 confirm*은 출력.)
