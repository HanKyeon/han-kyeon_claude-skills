...
Skills (C:\Users\<you>\.claude\skills):
  asset-factory            managed@0.25.0  →  [/cfh-make]
  debug-investigator       managed@0.25.0  →  [/cfh-debug]
  grilling                 managed@0.25.0  →  [/cfh-grill]
  cfh-harness          managed@0.25.0  →  [/cfh-team]
  refactoring-strategy     managed@0.25.0  →  [/cfh-refactor, /cfh-refactor-gen]
  skill-author             managed@0.25.0  →  [/cfh-new]
  tdd-first                managed@0.25.0  →  [/cfh-tdd, /cfh-tc]
  tdd-general              managed@0.25.0  →  [/cfh-tdd-gen, /cfh-tc-gen]
...
Commands (C:\Users\<you>\.claude\commands):
  ...
  cfh-retro, cfh-review, cfh-tc, cfh-tc-gen, cfh-tdd, cfh-tdd-gen, cfh-team, cfh-trace
                                          (모두 managed@0.25.0 · *=experimental)
```
...
`/cfh-team` + `cfh-harness`가 생성하는 multi-agent 팀의 6 패턴 (자세한 결정 트리는 `skills/cfh-harness/SKILL.md`):
...
- `~/.claude/skills/cfh-harness/SKILL.md` — 팀 생성 메타-스킬 + 6 패턴 (Pre-scan + 기본 6Q + Deep-dive 3지선 + Sanity R1~R8)
