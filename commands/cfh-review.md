...
기본 포함:
  ...
  [F] Project Health  — 기술 부채·모듈 경계·의존성·migration
  [G] Product Impact  — UX·메트릭·롤백·과잉 엔지니어링
...
답변: 전부 실행 / X 제외 / Y만 실행 / 추천대로(Enter)
```

**입력 해석 규칙**:
- `"전부 실행"` → 모두 포함 (제외 없음)
- `"추천대로"` / Enter → 📌 추천 그대로 적용
...
## Step 2.7: Communication Mode Selection
...
- **"프로젝트 규칙 우선" 원칙**
- **"REVIEW.md를 절대 직접 write/edit하지 말 것 — 결과는 응답 텍스트로만 반환"** (여러 agent가 직접 쓰면 마지막 agent가 전체를 덮어씀 — 실제 사고 사례. 파일 생성은 오케스트레이터가 Step 5에서 단독 수행)

### 모델 차등 (권장 — 강제 아님)

안전망 역할(틀리면 치명적)과 단순 스캔을 구분해 모델 tier를 *권장*한다 (sdk-ai-workflow: 안전망 리뷰=정밀 모델 / 생성=빠른 모델):
- **정밀 모델 권장**: Logic(B)·Security(E) — 놓치면 사고로 직결되는 안전망 축
- **빠른 모델 가능**: Convention(A)처럼 *정적 룰 대조* 위주의 단순 스캔
- Agent 도구의 `model` 파라미터로 subagent별 지정 가능

⚠ **한계 (과대평가 금지)**: cfh는 사용자 환경 모델에 의존 — 강제 못 하고 *"권장"까지*. 환경에 정밀 모델이 없으면 단일 모델로 진행해도 무방. 모델 차등은 *있으면 좋은* 최적화지 필수 게이트가 아님.
...
### Subagent F: 🏗️ Project Health Review (Medium+ 에서)
...
### Subagent G: 🎯 Product Impact Review (Medium+ 에서)
...
## Step 4.5: 실패 예산 (Failure Budget) — bounded 재시도 + 조용한 누락 금지

### ① finding 인용 검증 (통합 직전)

모든 finding의 **file:line 인용을 Read로 실제 코드와 대조** (해당 라인 주변만 targeted read — 추측 인용을 기계 대조로 승격, `structured-crosscheck.md`와 같은 정신):
- 일치 → finding 유지
- 불일치 → 해당 축 서브에이전트 **1회 재시도** → 재실패 시 **finding 폐기**
- REVIEW.md에 `검증 실패로 제외: N건` 명시 (0건이면 생략 — slot ≠ purpose)

### ② 서브에이전트 실행 실패

실행 실패(에러·빈 결과) → **1회 재시도** → 재실패 시 REVIEW.md에 `⚠ ○○ 축 리뷰되지 않음` 명시.
**조용한 누락 금지** — 커버리지 구멍은 보이게. 빠진 축을 다른 축 결과의 재해석으로 채우지 말 것.

> 재실행 간 "반복 지적" 추적은 **하지 않는다** — 이슈가 남아 있으면 다시 잡히는 게 정직한 출력이고, run 사이의 수렴 판단은 사람의 몫 (REVIEW.md는 일회성 재생성 리포트 — run 간 상태 없음).
...
| Performance | 0 | 0 | 0 | 0 |
| Security | 0 | 0 | 0 | 0 |
| **Project Health** | 0 | 0 | 0 | 0 |
| **Product Impact** | 0 | 0 | 0 | 0 |
...
## 🏗️ Project Health Review (Medium+ 에서만)
...
## 🎯 Product Impact Review (Medium+ 에서만)
