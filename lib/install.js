...
async function installOne({ src, dest, kind, name, link, force, dryRun, label }) {
  ...
  if (dryRun) {
    console.log(`  ~  [dry-run] ${link ? 'link' : 'copy'} -> ${label}`);
    return 'install';
  }
  ...
  if (link) {
    try {
      ...
      const isFile = src.endsWith('.md');
      if (isFile) {
        await fsp.symlink(src, dest, 'file');
      } else {
        await fsp.symlink(src, dest, linkType);
      }
      console.log(`  +  Linked: ${label}`);
      return 'install';
    } catch (err) {
      if (err.code === 'EPERM' || err.code === 'EACCES') {
        console.warn(`  !  Symlink failed for ${label} (permission). Falling back to copy.`);
...
async function install({ link, force, dryRun, target, skills, only }) {
  ...
  console.log(`Mode: ${link ? 'symlink' : 'copy'}${dryRun ? ' (dry-run)' : ''}`);
