...
- **좋은 테스트 품질**을 지킨다 (→ `commands/references/test-quality.md`): 리트머스(구현 바꿔도 행동 같으면 green)·피라미드(싼 계층 우선)·부작용 테스트(cleanup·blob·race — 리팩터 안전망)·의도 명시(회귀 주석·미커버 JSDoc). FE 쿼리 우선순위·`userEvent`·시간 고정.
</s>
...
- 기존 테스트 파일 상단 **의도 JSDoc** *있으면* 읽기 — 원래 의도와 충돌하는 테스트 추가 방지
